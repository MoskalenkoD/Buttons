...
let EditHtml = {
    ...
    creatingBlockForFigure: (node) => {
        ...
        let blockForFigure = NEW_BLOCK_DIV.cloneNode();
        blockForFigure.id = `id${Math.round(Math.random() * 10000).toString(16)}`;
        blockForFigure.classList.add('block-for-figures');
        ...
        let contentBlockForFigure = NEW_BLOCK_DIV.cloneNode();
    ...
    openModalWindow: () => {
        let htmlShadow = NEW_BLOCK_DIV.cloneNode();
        htmlShadow.className = 'shadow-for-form';
        htmlShadow.onclick = EditHtml.closeModal();

        let htmlFormForCreateFigure = NEW_BLOCK_DIV.cloneNode();
